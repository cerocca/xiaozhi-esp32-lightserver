...
def _load_config_data_for_update() -> tuple[dict | None, str]:
    try:
        ...
        return None, f"Error: unable to read the current config. {e}"

    try:
        data = yaml.safe_load(content) or {}
    except yaml.YAMLError as e:
        return None, f"Error: current config is invalid. {e}"
...
def test_active_llm(prompt_text: str = "Reply with OK.") -> dict:
    ...
    if not profile_name:
        result["ok"] = False
        result["message"] = "Error: no active LLM profile to test."
        result["error_reason"] = "runtime.llm_profile not resolved"
        return result

    if not endpoint:
        result["ok"] = False
        result["message"] = f"Error: LLM test not run: {profile_name}."
    ...
    try:
        ...
        result["ok"] = False
        result["message"] = f"Error: LLM test failed: {profile_name}."
        try:
            data = response.json()
        except ValueError:
            data = {}
...
def validate_llm_input(provider: str, api_key: str, model: str, base_url: str) -> dict:
    if provider not in PROVIDER_PRESETS:
        return {"ok": False, "message": f"Error: unsupported provider: {provider}."}

    if not model.strip():
        return {"ok": False, "message": "Error: model cannot be empty."}

    if not base_url.strip():
        return {"ok": False, "message": "Error: base URL cannot be empty."}
...
def set_active_llm(profile_name: str) -> dict:
    ...
    if not profile_name:
        return {"ok": False, "message": "Error: invalid LLM profile."}
...
def update_single_provider(
    ...
    if provider_id not in PROVIDER_PRESETS:
        return {"ok": False, "message": f"Error: unsupported provider: {provider_id}."}
...
def create_provider_profile(provider_id: str, profile_name: str | None = None) -> dict:
    ...
    while final_profile_name in llm_section:
        if requested_profile_name:
            return {"ok": False, "message": f"Error: profile already exists: {final_profile_name}."}
